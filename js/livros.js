const Livros = (() => {
  ...
  function gerarEstrelasHTML(valor, editavel = false) {
    ...
    for (let i = 1; i <= 5; i++) {
      ...
      if (editavel) {
        html += `<i class="${classe} estrela-editavel" data-valor="${i}" style="cursor:pointer; color:#f59e0b; font-size:1.2rem;"></i>`;
      } else {
        html += `<i class="${classe}" style="color:#f59e0b; font-size:1.2rem;"></i>`;
      }
    }
    return html;
  }
