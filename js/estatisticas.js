/**
 * Módulo Estatísticas – Eder Livros
 * Inclui gráficos de finalizados por mês, páginas por dia, gêneros,
 * dia da semana, velocidade de leitura, heatmap, além de cards e insights.
 */
const Estatisticas = (() => {
  const graficos = {};
  ...
  function criarGraficoVelocidade(dados) {
    ...
    graficos.velocidade = new Chart(ctx, {
      type: 'line',
      data: {
        labels: dados.map(d => d.dia),
        datasets: [{
          label: 'Páginas/hora',
          data: dados.map(d => d.velocidade),
          borderColor: '#10b981',
